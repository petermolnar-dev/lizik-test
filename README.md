...
2) Open the `UIDesignPatternsChallenge.xcodeproj` project on Xcode 15.3.
...
10) The MVVM and MVP modules contain commented-out snapshot tests at `MV\* Module/Tests/Feed UI/FeedUISnapshotTests.swift`.
	...
	- ⚠️ Important: ***Different simulators may generate slightly different snapshots (even if they look the same!).*** So you must run the snapshot tests using the exact same simulator used to take the snapshots:
		...
		- Use *precisely* the 'iPhone 15 Pro - iOS 17.4' simulator.
		...
		- If after resetting the tests fail on a fresh clone of this repository, make sure your 'iPhone 15 Pro - iOS 17.4' simulator language is set to English to match the CI environment: `Simulator > Settings > General > Language & Region` - it should have ONLY English as a language. Remove any other languages."
